...
public class DashboardWidgetAccessService {
    ...
    public Map<String, Boolean> getAllowedWidgetMap(Authentication authentication) {
    ...
    public Map<String, Boolean> getVisibleWidgetMap(Authentication authentication) {
        Map<String, Boolean> allowedWidgets = getAllowedWidgetMap(authentication);
        Set<String> hiddenWidgets = hiddenWidgetKeys(authentication);

        Map<String, Boolean> result = new LinkedHashMap<>();
        for (Map.Entry<String, Boolean> entry : allowedWidgets.entrySet()) {
            result.put(entry.getKey(), Boolean.TRUE.equals(entry.getValue()) && !hiddenWidgets.contains(entry.getKey()));
        }

        return result;
    }

    public List<DashboardWidgetDefinition> getAllowedWidgetDefinitions(Authentication authentication) {
        Map<String, Boolean> allowedWidgets = getAllowedWidgetMap(authentication);
        return definitions().stream()
                .filter(definition -> Boolean.TRUE.equals(allowedWidgets.get(definition.key())))
                .toList();
    }
    ...
    private Set<String> hiddenWidgetKeys(Authentication authentication) {
        String username = username(authentication);
        if (username.isBlank()) {
            return Set.of();
        }

        return userSettingRepository.findByUsername(username).stream()
                .filter(DashboardWidgetUserSetting::isHidden)
                .map(DashboardWidgetUserSetting::getWidgetKey)
                .collect(Collectors.toCollection(LinkedHashSet::new));
    }
