...
public class PlatformSettingService {
    ...
    /**
     * Ensure backend branding settings exist so the private area can follow
     * the active client's identity without hardcoding templates.
     */
    public void ensureDefaultsForAdminBranding() {
        String platformName = get("platform.name", businessProperties.getName());
        String platformLogo = get("platform.logo", businessProperties.getAdminLogo());
        String publicHeroBackground = get("public.hero.background_image", "");

        ensure("admin.brand.title", platformName,
                "string", "public_site", "Nombre mostrado en el encabezado del backend");
        ensure("admin.brand.subtitle", businessProperties.getAdminTitle(),
                "string", "public_site", "Subtítulo mostrado debajo del nombre del backend");
        ensure("admin.brand.logo", platformLogo,
                "image", "public_site", "Logo mostrado en el menú lateral del backend");
        ensure("admin.home.background_image", publicHeroBackground,
                "image", "public_site", "Imagen de fondo del inicio administrativo");
        ensure("admin.home.background_color", "#f3f5f9",
                "string", "public_site", "Color de fondo del inicio administrativo");
        ensure("admin.home.background_overlay", "rgba(243,245,249,0.88)",
                "string", "public_site", "Capa sobre la imagen del inicio administrativo");
    }

    /**
     * Ensure login settings exist so each client can customize the access screen
     * from the existing platform settings administration.
     */
    public void ensureDefaultsForLoginAccess() {
        ensure("login.background_image", "/img/login-bg2.png",
                "image", "public_site", "Imagen de fondo de la pantalla de acceso");
        ensure("login.logo", businessProperties.getAdminLogo(),
                "image", "public_site", "Logo de la pantalla de acceso");
        ensure("login.title", "Acceso",
                "string", "public_site", "Título de la pantalla de acceso");
        ensure("login.subtitle", "Ingresa con tu usuario para continuar.",
                "text", "public_site", "Subtítulo de la pantalla de acceso");
        ensure("login.username_label", "Usuario",
                "string", "public_site", "Etiqueta del campo usuario");
        ensure("login.username_placeholder", "Ingresa tu usuario",
                "string", "public_site", "Placeholder del campo usuario");
        ensure("login.password_label", "Contraseña",
                "string", "public_site", "Etiqueta del campo contraseña");
        ensure("login.password_placeholder", "Ingresa tu contraseña",
                "string", "public_site", "Placeholder del campo contraseña");
        ensure("login.remember_label", "Recordarme",
                "string", "public_site", "Etiqueta de recordar sesión");
        ensure("login.submit_label", "Acceder",
                "string", "public_site", "Texto del botón de acceso");
        ensure("login.forgot_label", "¿Olvidaste tu contraseña?",
                "string", "public_site", "Texto del enlace de recuperación");
        ensure("login.back_to_public_label", "Volver al portal",
                "string", "public_site", "Texto del enlace para volver al portal público");
        ensure("login.primary_color", "#0f766e",
                "string", "public_site", "Color principal de la pantalla de acceso");
        ensure("login.primary_hover_color", "#115e59",
                "string", "public_site", "Color hover de la pantalla de acceso");
        ensure("login.password_reset.request_title", "Recuperar contraseña",
                "string", "public_site", "Título de recuperación de contraseña");
        ensure("login.password_reset.request_text", "Ingresa tu usuario. Si existe y está activo, se generará un enlace temporal de recuperación.",
                "text", "public_site", "Texto de recuperación de contraseña");
        ensure("login.password_reset.reset_title", "Crear nueva contraseña",
                "string", "public_site", "Título para crear nueva contraseña");
        ensure("login.password_reset.submit_label", "Generar enlace de recuperación",
                "string", "public_site", "Botón para solicitar recuperación");
        ensure("login.password_reset.save_label", "Guardar nueva contraseña",
                "string", "public_site", "Botón para guardar nueva contraseña");
        ensure("login.password_reset.show_dev_link", "true",
                "boolean", "public_site", "Mostrar enlace temporal en desarrollo local");
    }
}
