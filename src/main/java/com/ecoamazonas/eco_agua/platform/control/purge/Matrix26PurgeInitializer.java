...
public class Matrix26PurgeInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        jdbcTemplate.execute("""
                CREATE TABLE IF NOT EXISTS matrix26_archive_destruction_plan (
                    id BIGINT NOT NULL AUTO_INCREMENT,
                    public_id VARCHAR(80) NOT NULL,
                    archive_record_id BIGINT NOT NULL,
                    archive_public_id VARCHAR(80) NOT NULL,
                    ...
                    last_error TEXT NULL,
                    PRIMARY KEY (id),
                    UNIQUE KEY uk_matrix26_archive_destruction_public (public_id),
                    KEY idx_matrix26_archive_destruction_archive (archive_record_id, status),
                    KEY idx_matrix26_archive_destruction_instance (instance_id, status),
                    KEY idx_matrix26_archive_destruction_backup (final_backup_job_id),
                    CONSTRAINT fk_matrix26_archive_destruction_archive
                        FOREIGN KEY (archive_record_id) REFERENCES matrix26_archive_record(id),
                    CONSTRAINT fk_matrix26_archive_destruction_instance
                        FOREIGN KEY (instance_id) REFERENCES platform_business_client(id)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
                """);

        jdbcTemplate.execute("""
                CREATE TABLE IF NOT EXISTS matrix26_archive_destruction_item (
                    id BIGINT NOT NULL AUTO_INCREMENT,
                    destruction_plan_id BIGINT NOT NULL,
                    run_number INT NOT NULL,
                    resource_type VARCHAR(80) NOT NULL,
                    resource_name VARCHAR(255) NULL,
                    resource_path VARCHAR(1000) NULL,
        ...
        jdbcTemplate.execute("""
                CREATE TABLE IF NOT EXISTS matrix26_archive_destruction_check (
                    ...
                    checked_at DATETIME(6) NOT NULL,
                    PRIMARY KEY (id),
                    KEY idx_matrix26_archive_destruction_check_plan (destruction_plan_id, run_number, status),
                    CONSTRAINT fk_matrix26_archive_destruction_check_plan
                        FOREIGN KEY (destruction_plan_id) REFERENCES matrix26_archive_destruction_plan(id)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
                """);

        jdbcTemplate.execute("""
                CREATE TABLE IF NOT EXISTS matrix26_archive_destruction_event (
                    id BIGINT NOT NULL AUTO_INCREMENT,
                    destruction_plan_id BIGINT NOT NULL,
                    event_type VARCHAR(100) NOT NULL,
                    status VARCHAR(30) NOT NULL,
                    ...
                    created_at DATETIME(6) NOT NULL,
                    PRIMARY KEY (id),
                    KEY idx_matrix26_archive_destruction_event_plan (destruction_plan_id, created_at),
                    CONSTRAINT fk_matrix26_archive_destruction_event_plan
                        FOREIGN KEY (destruction_plan_id) REFERENCES matrix26_archive_destruction_plan(id)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
                """);
    }
